...
void mapper_alloc(void *alloc, int size) {
    ...
    {   
        ...
        {
            fprintf(stderr, "Out of memory. Register a callback to handle OOM mapping failures: mapper_register_oom_callback(). Exiting. \n");
            exit(-1);
        }
    } else 
    {   
