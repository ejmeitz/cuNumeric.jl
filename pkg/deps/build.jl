...
function patch_legion(repo_root::String)
...
function build_cpp_wrapper(repo_root, conda_env_dir)
    ...
    #* CURRENTLY THROWS ERROR WHEN SETTING CMAKE_PREFIX_PATH
    # run(`export CMAKE_PREFIX_PATH=\"$conda_env_dir\"`)
    build_cmd = Cmd(`cmake -S $repo_root -B $build_dir`)
    build_cmd = setenv(build_cmd, "CMAKE_PREFIX_PATH" => conda_env_dir)
    run(build_cmd)
...
function core_build_process(conda_env_dir, run_legion_patch::Bool = true)
    ...
    # patch legion. The readme below talks about our compilation error
    # https://github.com/ejmeitz/cuNumeric.jl/blob/main/scripts/README.md
    run_legion_patch && patch_legion(repo_root)
...
conda_jl_env = load_preference("cuNumeric", "conda_jl_env", nothing)
user_env = load_preference("cuNumeric", "user_env", nothing)
